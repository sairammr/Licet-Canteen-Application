...
export default function MenuSection() {
  const { state, addToCart, removeFromCart, updateCartItemQuantity } = useApp();
  const [searchQuery, setSearchQuery] = useState('');
  const [sortBy, setSortBy] = useState<'name' | 'price' | 'preparationTime'>('name');

  const filteredAndSortedItems = state.menuItems
    .filter(item => 
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between">
                  {!isInCart ? (
                    ...
                    <div className="w-full flex items-center justify-between bg-gray-50 rounded-xl p-3">
                      <button
                        onClick={() => handleUpdateQuantity(item.id, cartQuantity - 1)}
                        className="w-10 h-10 rounded-full border-2 border-gray-300 flex items-center justify-center hover:border-black transition-colors bg-white"
                      >
                      ...
                      <span className="text-xl font-bold text-black px-4">{cartQuantity}</span>
